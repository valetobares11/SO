...
create(char *path, short type, short major, short minor)
{
  ...
  if((ip = dirlookup(dp, name, 0)) != 0){
    iunlockput(dp);
    ilock(ip);
    if(type == T_FILE && (ip->type == T_FILE || ip->type == T_DEVICE || ip->type == T_FIFO))
      return ip;
    iunlockput(ip);
    return 0;
  }
...
int sys_mkfifo(void) 
{
  ...
  // if (argint(1, &mode) < 0 && ((mode != RW_RW_RW) && (mode != RWX_RWX_RWX)))  
  //    panic("failure because it does not comply with the permits");
  //printf("%d\n ", mode);
